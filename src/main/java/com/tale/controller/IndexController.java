...
public class IndexController extends BaseController {
     ...
     * @return
     */
    @Route(value = "/spider", method = HttpMethod.POST)
    @JSON
    public RestResponse spiderArticle( Request request, Response response,@QueryParam String data) {

        if (!StringUtils.isBlank(data)) {
            JSONObject postJson = JSONObject.parseObject(data);
            if (postJson.get("userName") == null) {
                return RestResponse.fail(-1, "缺少上传文章用户必要参数");
            }
            String userId = postJson.getString("userName");
            String passWord = postJson.getString("password");
            Users user = usersService.login(userId,passWord);
            if (user == null) {
                return RestResponse.fail(-1, "错误");
            }
            try {
                boolean flag = contentsService.spiderPost(data, user);
                if (flag) {
                    siteService.cleanCache(Types.C_STATISTICS);
                    return RestResponse.ok(0);
                } else {
                    return RestResponse.fail(-1, "文章上传失败");
                }
            } catch (Exception e) {
                String msg = "文章上传失败";
                if (e instanceof TipException) {
        ...
        return RestResponse.fail(-1, "内容格式错误");
    }
}
